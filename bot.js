        ...
        if (state === 'add_group') {
            ...
            if (userStates[chatId].curatorStep === 'user_id') {
                ...
                if (user) {
                    ...
                    bot.sendMessage(chatId, `Пользователь с ID: ${user.telegram_id} назначен куратором группы с ID: ${groupId}.`);
                    userStates[chatId] = {};
                } else {
                    bot.sendMessage(chatId, 'Ошибка назначения куратора. Попробуйте еще раз.');
                }
            }
        }
    }
